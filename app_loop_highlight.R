  ...
  for(length_idx in 1:length(all_cycles)) {
    ...
    if(nrow(cycles_matrix) > 0) {
      for(i in 1:nrow(cycles_matrix)) {
        ...
        # 1. Total weight (sum of edge weights in the loop)
        total_weight <- 0
        # 2. Average node centrality in the loop
        avg_centrality <- 0
        # 3. Number of connections to other loops
        ...
        for(j in 1:length(cycle)) {
          ...
          neighbors <- neighbors(g, from_node)
          connections_to_other_loops <- connections_to_other_loops + 
            sum(!neighbors %in% cycle)
        }
...
# Function to identify and analyze loops
analyze_loops <- function(g) {
  # Find all cycles for each length from 3 to 6
  all_cycles <- lapply(3:6, function(n) {
    # Create a ring graph of length n for comparison
    ring <- make_ring(n, directed = TRUE)
    # Find subgraph isomorphisms (cycles)
    cycles <- try(graph.get.subisomorphisms.vf2(g, ring), silent = TRUE)
    
    if(!inherits(cycles, "try-error") && length(cycles) > 0) {
      # Convert the cycles list to a matrix
      cycles_matrix <- matrix(unlist(cycles), ncol = n, byrow = TRUE)
      return(cycles_matrix)
    }
    # Return empty matrix with correct number of columns
