...
analyze_loops <- function(g) {
  ...
  # Find all simple cycles using igraph's algorithms
  all_cycles <- lapply(3:6, function(n) {
    # Use all_simple_paths to find potential cycles
    cycles_list <- list()
    cycle_count <- 1
    
    # For each node, look for paths back to itself
    for(start_node in 1:vcount(g)) {
      # Find paths of length n-1 from this node to all other nodes
      possible_ends <- neighbors(g, start_node, mode="out")
      
      for(end_node in possible_ends) {
        # Check if there's a path of length n-2 from end_node back to start_node
        paths <- all_simple_paths(g, from=end_node, to=start_node, mode="out", cutoff=n-2)
        
        for(path in paths) {
          # Complete the cycle by adding the start node
          cycle <- c(start_node, path)
          
          # Check if it's the right length and forms a proper directed cycle
          if(length(cycle) == n && is_directed_cycle(cycle, g)) {
            cycles_list[[cycle_count]] <- cycle
    ...
    # Convert list of cycles to matrix
    if(length(cycles_list) > 0) {
      # Remove duplicate cycles (cycles that are rotations of each other)
      unique_cycles <- unique(lapply(cycles_list, function(x) {
        min_pos <- which.min(x)
        c(x[min_pos:length(x)], x[1:(min_pos-1)])
      }))
      
      cycles_matrix <- do.call(rbind, unique_cycles)
    } else {
      cycles_matrix <- matrix(numeric(0), nrow=0, ncol=n)
    }
  ...
  loop_metrics <- data.frame(
    Loop_Length = 3:6,
    Count = sapply(all_cycles, nrow),
    stringsAsFactors = FALSE
  )
  
  # Identify nodes frequently involved in loops
  all_loop_nodes <- unlist(lapply(all_cycles, function(x) {
    if(is.matrix(x) && nrow(x) > 0) as.vector(x) else numeric(0)
  }))
  
  # Create node frequency table
  node_freq_df <- data.frame(
    Node = 1:vcount(g),
    Times_in_Loops = 0
  )
  
  if(length(all_loop_nodes) > 0) {
    node_counts <- table(all_loop_nodes)
    node_freq_df$Times_in_Loops[as.numeric(names(node_counts))] <- as.numeric(node_counts)
  }
  
  list(
    cycles = all_cycles,
    metrics = loop_metrics,
    node_frequencies = node_freq_df
  )
}
